...
execute if score @s twis.cst_idle >= $min_cst_idle twis.data if score @s twis.cst_idle < $max_cst_idle twis.data unless score @s twis.cst_playing matches 0.. if predicate immersive:cst/play_chance run function immersive:cst/check
execute if score @s twis.cst_idle >= $max_cst_idle twis.data unless score @s twis.cst_playing matches 0.. run function immersive:cst/check

# Instant check (Dragon Battle)
execute unless score @s twis.cst_playing matches 0.. if predicate immersive:dimension/in_end if entity @e[type=minecraft:ender_dragon] run function immersive:cst/check

# Increase playing by 1 every second /  End soundtrack and reset idle if playing score >= length score
execute if score @s twis.cst_playing matches 0.. if score @s twis.cst_playing >= @s twis.cst_length run function immersive:cst/ended
